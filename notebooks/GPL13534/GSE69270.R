...
# Setting path
###############################################
path_data <- "D:/YandexDisk/Work/pydnameth/datasets/GPL13534/GSE69270/raw"
...
path_work <- "D:/YandexDisk/Work/pydnameth/draft/10_MetaEPIClock/MetaEpiAge/GPL13534/GSE69270"
setwd(path_work)
...
row.names(pd) <- gsub("leukocyte_", "", pd$title)
betas <- as.data.frame(read.table(paste(path_data,"/GSE69270_Processed450BeadchipData_Beta_values_with_Pvalues_YFS_KananenL.txt", sep=""), header=TRUE, sep='\t'))
...
mvals_ref <- logit2(betas_ref)
mvals_ref <- data.frame(rownames(mvals_ref), mvals_ref)
colnames(mvals_ref)[1] <- "ID_REF"
mvals_ref <- regRCPqnREF(M_data=mvals_ref, ref_path=path_harm_ref, data_name=dataset_ref)
betas_ref <- ilogit2(mvals_ref)
