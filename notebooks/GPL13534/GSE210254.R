...
path_work <- "D:/YandexDisk/Work/pydnameth/draft/10_MetaEPIClock/MetaEpiAge/GPL13534/GSE210254"
setwd(path_work)
...
row.names(pd) <- gsub("sample","", as.character(pd$title))

betas <- as.data.frame(read.table(paste(path_data,"/GSE210254_Beta_value_450K.txt", sep=""), header=TRUE))
rownames(betas) <- betas$CpGnames
