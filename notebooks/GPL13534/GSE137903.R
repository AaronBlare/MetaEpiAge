...
# Setting path
###############################################
path_data <- "D:/YandexDisk/Work/pydnameth/datasets/GPL13534/GSE137903/raw"
...
path_work <- "D:/YandexDisk/Work/pydnameth/draft/10_MetaEPIClock/MetaEpiAge/GPL13534/GSE137903"
setwd(path_work)
...
row.names(pd) <- paste0('X', pd$title)

betas <- as.data.frame(read.table(paste(path_data,"/GSE137903_matrix_processed_GECKO.txt", sep=""), header=TRUE))
col_odd <- seq_len(ncol(betas)) %% 2
betas <- betas[ , col_odd == 1]
colnames(betas) = gsub("_avgbeta", "", colnames(betas))
