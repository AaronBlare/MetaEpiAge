...
path_work <- "D:/YandexDisk/Work/pydnameth/draft/10_MetaEPIClock/MetaEpiAge/GPL13534/GSE157131"
setwd(path_work)
...
row.names(pd) <- pd$description

data <- as.data.frame(read.table(paste(path_data,"/GSE157131_Matrix_raw_data_450K_08252020.txt", sep=""), header=TRUE))
rownames(data) <- data$ID_REF
data <- within(data, rm(ID_REF))
col_ids <- seq_len(ncol(data)) %% 3
meth <- data[ , col_ids == 1]
unmeth <- data[ , col_ids == 2]
betas <- meth / (meth + unmeth + 100)
pd$names_from_raw <- colnames(betas)
sentrixids <- cSplit(pd, "names_from_raw", ".")
pd$names_from_raw <- sentrixids$names_from_raw_1
colnames(betas) <- pd$names_from_raw
